...
export class SettingsHandler {
  ...
  async initialize(hostname: string): Promise<void> {
    ...
    if (!this.targetHostname) {
      console.error(
        `SettingsHandler (Target: ${this.targetHostname}): Initialization aborted - no valid target hostname.`
      );
      this.currentSettings = { ...defaultSettings };
    ...
    try {
      ...
      if (response && response.success && response.settings) {
        ...
        console.log(
          `SettingsHandler (Target: ${this.targetHostname}): Applied initial settings:`,
          JSON.stringify(this.currentSettings)
        );
      } else {
        this.currentSettings = { ...defaultSettings };
        console.warn(
          `SettingsHandler (Target: ${this.targetHostname}): No valid settings in response. Using defaults. Response:`,
          response
        );
      }
    } catch (error) {
      this.currentSettings = { ...defaultSettings };
      console.error(
        `SettingsHandler (Target: ${this.targetHostname}): Error during GET_INITIAL_SETTINGS:`,
        error,
        "Using defaults."
      );
    } finally {
  ...
  updateSettings(settings: AudioSettings): void {
    console.log(
      `SettingsHandler (Target: ${this.targetHostname}): Settings updated locally`,
      settings
    );
